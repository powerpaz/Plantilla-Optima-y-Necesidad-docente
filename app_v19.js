...
(function(){
  ...
  function findSheetName(wb, desired){
    const want = normalizeSheetName(desired);
    for (const n of wb.SheetNames){
      if (normalizeSheetName(n) === want) return n;
    }
    for (const n of wb.SheetNames){
      if (normalizeSheetName(n).includes(want)) return n;
    }
    return null;
  }
