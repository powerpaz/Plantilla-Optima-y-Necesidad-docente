...
(function(){
  ...
  function findSheetName(wb, wanted) {
    ...
    for (const s of wb.SheetNames) {
      if (normalizeSheetName(s) === target) return s;
    }
    return null;
  }

  function getSheetData(wb, sheetName) {
    const realName = findSheetName(wb, sheetName);
    if (!realName) return null;
    const sheet = wb.Sheets[realName];
    return window.XLSX.utils.sheet_to_json(sheet, { header: 1, defval: null, raw: false });
  }
   ...
   */
  function validateParPO(parPOData) {
    const issues = [];
    
    // VALIDACIÓN ADICIONAL: Obligatorios desde A3 (no tocar celdas automáticas - solo lectura)
    function isEmpty(v) {
    ...
    // Validar AMIE
    let amieRow = parPOData.find(row => 
      row && row[0] && String(row[0]).trim().toUpperCase().includes('AMIE')
    );
    
    if (amieRow) {
      const amie = amieRow[1];
      if (!amie || amie === 0 || amie === '0') {
        issues.push({
          severity: "error",
          sheet: "Par_PO",
          paso: "8",
          row: parPOData.indexOf(amieRow) + 1,
          message: "Falta código AMIE en Par_PO"
        });
      }
    }
    ...
    const camposObligatorios = [
      { nombre: 'NOMBRE DE LA IE', paso: '8' },
      { nombre: 'MODALIDAD', paso: '8' },
      { nombre: 'TIPO DE EDUCACIÓN', paso: '8' },
      { nombre: 'AÑO LECTIVO', paso: '8' },
      { nombre: 'CATEGORÍA', paso: '8' }
    ];
    
    camposObligatorios.forEach(campo => {
      const row = parPOData.find(r => 
        r && r.some(cell => cell && String(cell).toUpperCase().includes(campo.nombre))
      );
      
      if (row) {
        const valueIndex = row.findIndex(cell => cell && String(cell).toUpperCase().includes(campo.nombre)) + 1;
        ...
        if (!value || value === 0 || value === '0') {
          issues.push({
            ...
            sheet: "Par_PO",
            paso: campo.paso,
            row: parPOData.indexOf(row) + 1,
            message: `Falta completar ${campo.nombre} (Paso ${campo.paso})`
          });
        }
      }
    });
    
    // Paso 9: Validar que hay datos de paralelos
    const paralelos = parPOData.find(row => 
      row && row.some(cell => cell && String(cell).toLowerCase().includes('paralelos'))
    );
    ...
    if (!paralelos) {
      issues.push({
        ...
        paso: "9",
        row: 0,
        message: "No se encontró información de paralelos (Paso 9)"
      });
    }

    
    // A3 en adelante: validar datos obligatorios en la tabla (A-E)
    // Nota: evitamos falsos positivos deteniendo el escaneo tras 10 filas vacías consecutivas.
    let blankStreak = 0;
    for (let r = 2; r < Math.min(parPOData.length, 500); r++) { // r=2 => Excel fila 3
      const row = parPOData[r] || [];
      const A = row[0], B = row[1], C = row[2], D = row[3], E = row[4];
      const any = !isEmpty(A) || !isEmpty(B) || !isEmpty(C) || !isEmpty(D) || !isEmpty(E);
      if (!any) {
        blankStreak++;
      ...
      if (isEmpty(A)) issues.push({ severity: "warning", sheet: "Par_PO", step: 8, row: r + 1, description: "Falta dato obligatorio en A (Grado/Curso) desde A3." });
      if (isEmpty(C)) issues.push({ severity: "warning", sheet: "Par_PO", step: 8, row: r + 1, description: "Falta dato obligatorio en C (Jornada) desde A3." });
      if (isEmpty(D)) issues.push({ severity: "warning", sheet: "Par_PO", step: 8, row: r + 1, description: "Falta dato obligatorio en D (Paralelo) desde A3." });
      if (isEmpty(E)) issues.push({ severity: "warning", sheet: "Par_PO", step: 8, row: r + 1, description: "Falta dato obligatorio en E (Nro. estudiantes) desde A3." });
    }

    // L13 en adelante: verificar que esté tipeada la información (columna L)
    blankStreak = 0;
    for (let r = 12; r < Math.min(parPOData.length, 800); r++) { // r=12 => Excel fila 13
      const row = parPOData[r] || [];
      const A = row[0];
      const L = row[11]; // columna L
      const active = !isEmpty(A) || !isEmpty(L);
      if (!active) {
        blankStreak++;
        if (blankStreak >= 10) break;
        continue;
      ...
      blankStreak = 0;
      if (isEmpty(L)) issues.push({ severity: "warning", sheet: "Par_PO", step: 8, row: r + 1, description: "Desde L13 debe estar ingresada la información en columna L (no vacío)." });
    }
  ...
  function validateDisTra(disTraData) {
    ...
    dataRows.forEach((row, idx) => {
      ...
      if (colTotal !== -1) {
        ...
        // Validar que no exceda las 25 horas (LOEI reformada)
        if (total && Number(total) > 25) {
          issues.push({
            severity: "warning",
            sheet: "DIS_TRA",
            paso: "11",
            row: actualRow,
            message: `Total de horas (${total}) excede las 25 horas pedagógicas semanales`
          });
        }
      }
    });
  ...
  function displayWorkflowSteps(pasos) {
    if (pasos.length === 0) {
      $("workflowSteps").innerHTML = '<p class="muted">No se encontró hoja "Pasos" con el flujo de trabajo</p>';
      return;
    }
