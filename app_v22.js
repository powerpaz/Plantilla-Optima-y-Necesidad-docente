(function(){
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    $('btnInterpret').addEventListener('click', showInterpretModal);
    $('btnCloseInterpret').addEventListener('click', () => $('interpretModal').style.display='none');
    $('btnCopyInterpret').addEventListener('click', copyInterpretText);
    $('interpretModal').addEventListener('click', e => {
      if (e.target === $('interpretModal')) $('interpretModal').style.display='none';
    });
  });
  ...
  function validateDistra(amie, fileName, ws, parInfo, issues){
    ...
    // ── Detectar fila que contiene "Tot_General" y los nombres de paralelo ──
    // En Chimborazo: Excel fila 9 (0-indexed r=8); J9=Tot_General, K9..=nombres de paralelos
    let nameRowR = -1, totGeneralC = -1;
    for (let r = 5; r <= 14 && nameRowR < 0; r++){
      for (let c = 0; c <= 12; c++){
        const v = normalize(getCellDisplay(ws, XLSX.utils.encode_cell({r,c}))).toLowerCase();
        if (v.includes('tot_general') || v.includes('tot general')){
          nameRowR = r;
          totGeneralC = c;
          break;
        }
    ...
    if (nameRowR < 0)  nameRowR  = 8; // fallback: Excel fila 9 (0-indexed)
    if (totGeneralC < 0) totGeneralC = 9; // fallback: col J, confirmado en Chimborazo

    // ── Detectar fila de "Tronco común" ──
    // En Chimborazo: Excel fila 6 (0-indexed r=5); J6="Tronco común"
    let troncoRowR = -1;
    for (let r = 3; r <= 9 && troncoRowR < 0; r++){
      for (let c = 7; c <= 12; c++){
        const v = normalize(getCellDisplay(ws, XLSX.utils.encode_cell({r,c}))).toLowerCase();
        if (v.includes('tronco')){ troncoRowR = r; break; }
      }
    }
    if (troncoRowR < 0) troncoRowR = 5; // fallback: Excel fila 6 (0-indexed)

    // ── Detectar rango de columnas de paralelos (desde col K, índice 10) ──
    let structureCols = 0, firstDataC = -1;
    const scanRows = [troncoRowR, troncoRowR+1, nameRowR-1, nameRowR];
    for (let c = 10; c <= 250; c++){
      const hasContent = scanRows.some(r =>
    ...
    // ── Identificar columnas BT desde fila de nombres de paralelo ──
    // Un paralelo es BT si su nombre contiene "Bachillerato"
    const btColumns = new Set();
    if (firstDataC >= 0){
      for (let c = firstDataC; c < firstDataC + structureCols; c++){
        const v = normalize(getCellDisplay(ws, XLSX.utils.encode_cell({r:nameRowR, c}))).toLowerCase();
        if (v.includes('bachillerato')) btColumns.add(c);
      }
    }

    // ── Validar tronco común ──
    // Regla PCEI BT (Acuerdo MINEDUC-2024-00065-A, confirmado en hoja BT_ fila 20):
    //   tronco común = 0 en todos los paralelos BT. Los estudiantes PCEI completaron
    //   el tronco en la fase EGB y acceden directamente a los módulos técnicos.
    // Regla no-BT: alerta si valor > 25 períodos (Art. 192 Reglamento LOEI).
    if (firstDataC >= 0){
      for (let c = firstDataC; c < firstDataC + structureCols; c++){
        const addr = XLSX.utils.encode_cell({r:troncoRowR, c});
        ...
        if (btColumns.has(c)){
          if (val !== 0)
            issues.push(issue('error',amie,fileName,'DIS_TRA', addr,
              `Columna ${colLetter(c+1)} es paralelo BT y tiene tronco común = ${val} h. ` +
              `En PCEI con BT el tronco debe ser 0: los estudiantes completaron el tronco en la ` +
              `fase EGB y solo cursan los módulos técnicos. ` +
              `Plan de estudios Acuerdo MINEDUC-2024-00065-A (BT_ fila 20).`));
        } else {
          if (val > 25)
            issues.push(issue('warning',amie,fileName,'DIS_TRA', addr,
              `Tronco común columna ${colLetter(c+1)} = ${val} períodos. ` +
              `Supera el máximo de 25 períodos semanales (Art. 192 Reglamento LOEI). Verificar.`));
        }
      }
    }
    ...
    // ── Carga horaria por docente (columna Tot_General detectada dinámicamente) ──
    // Col B (índice 1) = Nombres; Tot_General = columna detectada (col J en Chimborazo)
    let teachersWithLoad = 0, maxHours = 0, overloadCount = 0;

    for (let r0 = nameRowR + 1; r0 <= 500; r0++){
      ...
      const totVal = toNumber(getCellDisplay(ws, XLSX.utils.encode_cell({r:r0, c:totGeneralC})));
      if (isNaN(totVal) || totVal <= 0) continue;

      teachersWithLoad++;
      if (totVal > maxHours) maxHours = totVal;

      if (totVal > MAX_HORAS_DOCENTE){
        overloadCount++;
        issues.push(issue('error',amie,fileName,'DIS_TRA',`B${r0+1}`,
          `"${nameVal}": ${totVal} períodos/semana — excede el límite de ${MAX_HORAS_DOCENTE}. ` +
          `Art. 117 LOEI: "25 períodos pedagógicos semanales de clase" es el máximo.`));
      }
    }

    // Fallback si Tot_General no dio resultados: sumar columnas de paralelos
    if (teachersWithLoad === 0 && firstDataC >= 0){
      for (let r0 = nameRowR + 1; r0 <= 500; r0++){
        ...
        if (sumHours > MAX_HORAS_DOCENTE){
          overloadCount++;
          issues.push(issue('error',amie,fileName,'DIS_TRA',`B${r0+1}`,
            `"${nameVal}": ${sumHours} períodos/semana — excede el límite de ${MAX_HORAS_DOCENTE}. Art. 117 LOEI.`));
        }
      }
    }
  ...
  function renderReport(report){
    ...
    $('summaryCards').innerHTML = `
      ...
      <p class="small muted mt16">Versión: <span class="code">${escapeHtml(s.build)}</span>
        · LOEI Art.117 · Acuerdo 2021-00057-A · Acuerdo 2022-00034-A · Acuerdo 2024-00065-A
  ...
  function showInterpretModal(){
    if (!lastReport) return;
    $('interpretText').textContent = buildInterpretationText(lastReport);
    $('copyOkMsg').style.display = 'none';
    $('interpretModal').style.display = 'flex';
  }

  function copyInterpretText(){
    const text = $('interpretText').textContent;
    navigator.clipboard.writeText(text).then(() => {
      const ok = $('copyOkMsg');
      ok.style.display = 'inline';
      setTimeout(() => { ok.style.display = 'none'; }, 2800);
    }).catch(() => {
  ...
  function buildInterpretationText(report){
    ...
    L.push(SEP);
    L.push('REVISIÓN TÉCNICA AUTOMATIZADA — MATRICES DTD PCEI');
    L.push(`Fecha: ${fecha}   |   Sistema: PO-App ${BUILD}`);
    L.push(`Base normativa: LOEI Art. 117 · Acuerdo MINEDUC-2021-00057-A · Acuerdo MINEDUC-2022-00034-A · Acuerdo MINEDUC-2024-00065-A`);
    L.push(SEP);

    if (report.fileRows.length > 1){
      L.push('');
      const s = report.summary;
      L.push(`RESUMEN GENERAL: ${s.matricesLoaded} matrice(s) revisada(s)   ` +
        `${s.errors} con error normativo   ${s.warnings} con advertencia   ${s.ok} sin novedades`);
    }

    report.fileRows.forEach((row, idx) => {
      const fileIssues = report.allIssues.filter(i => i.fileName === row.fileName);
      const errors   = fileIssues.filter(i => i.level === 'error');
      const warnings = fileIssues.filter(i => i.level === 'warning');

      L.push('');
      L.push(SEP);
      if (report.fileRows.length > 1) L.push(`INSTITUCIÓN ${idx+1} DE ${report.fileRows.length}`);
      L.push(`AMIE: ${row.amie}`);
      L.push(`Archivo: ${row.fileName}`);
      L.push(SEP);

      L.push('');
      L.push('DATOS DE PLANTILLA ÓPTIMA (Art. 117 LOEI):');
      L.push(`  Plantilla Óptima calculada (Par_PO!E19) : ${row.po} plazas docentes`);
      L.push(`  Directivos (Par_PO!E18)                 : ${row.directivos}`);
      L.push(`  Necesidad declarada (Nómina!E11/E12)    : ${row.necesidad}`);

      L.push('');
      const resLabel = row.status === 'OK'
        ? 'SIN ERRORES NORMATIVOS — APROBADO'
        : row.status === 'ADVERTENCIA'
        ? 'CON OBSERVACIONES — SIN ERRORES NORMATIVOS (verificar antes de aprobar)'
        : 'CON ERRORES NORMATIVOS — REQUIERE CORRECCIÓN';
      L.push(`RESULTADO: ${resLabel}`);

      if (errors.length > 0){
        L.push('');
        L.push(sep2);
        L.push(`ERRORES NORMATIVOS — CORRECCIÓN OBLIGATORIA (${errors.length} encontrado${errors.length!==1?'s':''})`);
        L.push(sep2);
        L.push('Los siguientes errores constituyen incumplimiento de normativa vigente y');
        L.push('deben ser subsanados antes de la aprobación de la Plantilla Óptima.');
        errors.forEach((e, i) => {
          L.push('');
          L.push(`  ${i+1}. [${e.sheet}${e.cell ? ' · ' + e.cell : ''}]`);
          L.push(`     ${e.message}`);
        });
      }
      ...
      if (warnings.length > 0){
        L.push('');
        L.push(sep2);
        L.push(`OBSERVACIONES — VERIFICACIÓN REQUERIDA (${warnings.length} encontrada${warnings.length!==1?'s':''})`);
        L.push(sep2);
        warnings.forEach((w, i) => {
          L.push('');
          L.push(`  ${i+1}. [${w.sheet}${w.cell ? ' · ' + w.cell : ''}]`);
          L.push(`     ${w.message}`);
        });
      }
      ...
      L.push('');
      L.push(sep2);
      L.push('ACCIÓN REQUERIDA');
      L.push(sep2);
      if (errors.length > 0){
        L.push(`Se solicita a la institución educativa AMIE ${row.amie} realizar las correcciones`);
        L.push(`indicadas en los ${errors.length} error${errors.length!==1?'es':''}  normativo${errors.length!==1?'s':''} y remitir la matriz DTD`);
        L.push('corregida a esta Dirección para su revisión y aprobación.');
      } else if (warnings.length > 0){
        L.push('La matriz no presenta errores normativos. Se recomienda revisar y confirmar');
        L.push(`las ${warnings.length} observación${warnings.length!==1?'es':''} señaladas antes de la aprobación definitiva.`);
      } else {
        L.push('La matriz no presenta errores normativos ni observaciones.');
        L.push('Puede procederse con la aprobación de la Plantilla Óptima.');
      }
    });

    L.push('');
    L.push(SEP);
    L.push(`Generado automáticamente por PO-App ${BUILD} — Revisión DPI`);
    L.push('LOEI Art. 117  |  Acuerdo MINEDUC-2021-00057-A  |  Acuerdo MINEDUC-2022-00034-A  |  Acuerdo MINEDUC-2024-00065-A');
    L.push(SEP);
