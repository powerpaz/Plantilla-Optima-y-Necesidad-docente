/* PO-App v10
 * Novedades (v8 institucional, sin CDN):
...
const BUILD = { version: "10.0.0", date: "2026-03-04" };
const $ = (id) => document.getElementById(id);


// Estado de librerías (modo institucional sin CDN)
function checkLibsAndWarn(){
  ...
  if (missing.length){
    ...
    statusEl.innerHTML =
      "<b>Dependencias no cargadas:</b> " + missing.join(", ") +
      ".<br><span class='muted'>Build v" + BUILD.version +
      " · Este modo NO usa CDN: asegúrate de subir la carpeta <code>vendor/</code> completa al repo (junto a <code>index.html</code>). Luego recarga con <b>Ctrl+F5</b>. Si aún ves PO-App v7, GitHub Pages está sirviendo una versión anterior.</span>";
    return false;
  }

  statusEl.style.display = "block";
  statusEl.className = "note";
  statusEl.innerHTML = "<b>Dependencias OK:</b> XLSX + jsPDF cargados localmente (sin CDN). <span class='muted'>Build v" + BUILD.version + "</span>";
  return true;
}
...
function setup(){
  ...
  // Verifica librerías locales (XLSX/jsPDF). Si falta vendor/, bloquea el flujo.
  const libsOk = checkLibsAndWarn();
  if (!libsOk) {
  ...
  excelInput.addEventListener("change", () => {
    ...
    btnProcesar.disabled = !(ok && checkLibsAndWarn());
  ...
  // Diagnóstico (para despliegue/caché)
  const btnDiag = 0 0"btnDiagnostico");
  if (btnDiag){
    btnDiag.addEventListener("click", () => {
      ...
      const diag = {
        ...
        hasJsPDF: typeof window.jspdf !== "undefined" || typeof jsPDF !== "undefined",
        location: window.location.href,
        scripts
      };
      alert("Diagnóstico PO-App\n\n" + JSON.stringify(diag, null, 2));
    });
  }
...
async function procesarExcel(){
  ...
  if (typeof XLSX === "undefined"){
    alert("No se cargó la librería XLSX (local).

Checklist rápido:
1) En tu repo, la carpeta vendor/ debe estar en la RAÍZ junto a index.html (no dentro de otra carpeta).
2) GitHub Pages debe publicar desde /(root) o /docs según tu configuración, pero el index.html que se publica debe ser el de PO-App v10.
3) Borra caché del sitio: Ctrl+F5 o modo incógnito. Si persiste: Configuración del navegador → Datos del sitio → borrar para powerpaz.github.io.

Si arriba aún dice PO-App v7, no se actualizó el despliegue." );
    return;
  }
...
function downloadPdf(report){
  try{
    ...
    alert("No se pudo generar PDF (verifica que jsPDF esté disponible (carpeta vendor)): " + String(e?.message || e));
  }
}
