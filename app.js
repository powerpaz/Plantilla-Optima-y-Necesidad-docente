...
// =========================
// CARGA RESILIENTE DE LIBRERÍAS (CDN fallback)
// =========================
...
async function ensureJsPDF(){
  ...
  const ok = await ensureGlobal('jspdf', [
  ...
  for (const u of pluginUrls){
    try{ await loadScript(u); break; }catch(_e){}
  }
  return true;
}
...
async function procesarExcel(){
  ...
  if (!(await ensureXLSX())){
    alert(
      "No se pudo cargar la librería XLSX desde CDN.\n\n"+
      "Causas comunes:\n"+
      "• Red institucional bloquea cdn.jsdelivr.net / cdnjs / unpkg\n"+
      "• Extensión de seguridad en el navegador\n\n"+
      "Solución rápida:\n"+
      "• Probar en otra red (hotspot) o permitir esos dominios."
    );
    return;
  }
...
function downloadPdf(report){
  if (typeof window.jspdf === 'undefined'){
    ensureJsPDF().then(ok => {
      if (!ok) alert('No se pudo cargar jsPDF desde CDN.');
